...
class DFINEHungarianMatcher(nn.Module):
    ...
    def forward(self, outputs: Dict[str, torch.Tensor], targets, return_topk=False):
        ...
        # Early, actionable error instead of a cryptic CUDA device-side assert.
        # Common failure: COCO-format datasets store category_id as 1..N but the model expects labels 0..N-1.
        if tgt_ids.numel() > 0:
            min_id = int(tgt_ids.min().item())
            max_id = int(tgt_ids.max().item())
            if min_id < 0 or max_id >= num_classes:
                raise ValueError(
                    "Target labels are out of range for matcher indexing: "
                    f"min={min_id}, max={max_id}, num_classes={num_classes}. "
                    "Ensure your dataset labels are contiguous 0..(num_classes-1). "
                    "For COCO-style datasets with category_id starting at 1, enable `remap_category_ids: True`."
                )
