...
class BaseConfig(object):
    ...
    def __init__(self) -> None:
        ...
        self._amp_dtype_str: str = "float16"
    ...
    def amp_dtype(self) -> torch.dtype:
        """AMP dtype for autocast. bfloat16 does not require GradScaler."""
        return torch.bfloat16 if self._amp_dtype_str == "bfloat16" else torch.float16

    @property
    def scaler(self) -> GradScaler:
        # GradScaler not needed for bfloat16 (better numerical range)
        if self._scaler is None and self.use_amp and torch.cuda.is_available() and self._amp_dtype_str != "bfloat16":
