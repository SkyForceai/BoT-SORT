...
class ELAN(nn.Module):
    ...
    # def forward(self, x): # Previously used - problem with software CV/TRT inference chunk
    #     # y = [self.cv1(x)]
...
class RepNCSPELAN4(nn.Module):
    ...
    # def forward(self, x): # Previously used - problem with software CV/TRT inference slice
    #     y = list(self.cv1(x).split((self.c, self.c), 1))
    #     y.extend(m(y[-1]) for m in [self.cv2, self.cv3])
    #     return self.cv4(torch.cat(y, 1)) 
    def forward(self, x):
        x1 = self.cv1(x) 
        y = [x1[:, :self.c, :, :], x1[:, self.c:, :, :]]  # Explicit slicing instead of split - more ONNX-friendly
        y.extend(m(y[-1]) for m in [self.cv2, self.cv3])
        return self.cv4(torch.cat(y, 1))
...
class DFINEHybridEncoder(nn.Module):
    ...
    def forward(self, feats):
        ...
        if self.num_encoder_layers > 0:
            for i, enc_ind in enumerate(self.use_encoder_idx):
                ...
                if self.training or self.eval_spatial_size is None:
                    # Training (or no fixed eval size): always generate positional embeddings
                    # matching the current feature map resolution.
                    pos_embed = self.build_2d_sincos_position_embedding(
                        w, h, self.hidden_dim, self.pe_temperature
                    ).to(src_flatten.device)
                else:
                    # Eval/deploy mode: prefer cached pos-embeds (built from eval_spatial_size),
                    # but fall back to dynamic generation if the cached shape doesn't match the
                    # current input resolution (e.g. FLOPs profiling uses base_size×base_size).
                    pos_embed = getattr(self, f"pos_embed{enc_ind}", None)
                    if pos_embed is None or pos_embed.shape[1] != src_flatten.shape[1]:
                        pos_embed = self.build_2d_sincos_position_embedding(
                            w, h, self.hidden_dim, self.pe_temperature
                        )
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            # Use explicit size (instead of scale_factor=2) to avoid off-by-one
            # mismatches when input H/W aren't divisible by the backbone stride.
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            if downsample_feat.shape[-2:] != feat_height.shape[-2:]:
                downsample_feat = F.interpolate(
                    downsample_feat, size=feat_height.shape[-2:], mode="nearest"
                )
