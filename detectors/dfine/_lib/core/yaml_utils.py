...
# Keys in YAML configs that are expected to be filesystem paths.
# These often appear inside dataset configs and were historically written relative to the YAML file,
# but interpreted relative to the current working directory at runtime.
_PATH_KEYS = {"ann_file", "img_folder"}


def _resolve_path_str(path_str: str, base_dir: str) -> str:
...
def _resolve_paths_in_cfg_inplace(obj: Any, base_dir: str) -> None:
    """Recursively resolve known path keys inside a loaded YAML structure."""
    if isinstance(obj, dict):
        for k, v in obj.items():
            ...
            if k in _PATH_KEYS:
                if isinstance(v, str):
                    obj[k] = _resolve_path_str(v, base_dir)
                elif isinstance(v, list):
                    obj[k] = [_resolve_path_str(p, base_dir) if isinstance(p, str) else p for p in v]
                else:
                    obj[k] = v
            else:
                _resolve_paths_in_cfg_inplace(v, base_dir)
        return

    if isinstance(obj, list):
        for item in obj:
            _resolve_paths_in_cfg_inplace(item, base_dir)
...
def load_config(file_path, cfg=dict()):
    ...
    _resolve_paths_in_cfg_inplace(file_cfg, base_dir=os.path.dirname(file_path))
