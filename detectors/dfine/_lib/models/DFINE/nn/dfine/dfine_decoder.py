...
class LQE(nn.Module):
    ...
    # def __init__(self, k, hidden_dim, num_layers, reg_max, exponents=None, eps=1e-6):
    #     super().__init__()
    #     self.k = k
    #     self.reg_max = reg_max
    #     self.eps = eps

    #     # Choose exponents that progressively approximate "topness"
    #     # Example for k=4: [2, 4, 8, 16]
    #     if exponents is None:
    #         exponents = [2 ** (i + 1) for i in range(k)]
    #     assert len(exponents) == k
    #     self.register_buffer("_exps", torch.tensor(exponents, dtype=torch.float32), persistent=False)
    ...
    #     prob = F.softmax(pred_corners.reshape(B, L, 4, N), dim=-1)  # [B,L,4,N]
    #     p = prob.clamp_min(self.eps)

    #     # Broadcast exponents to match prob dtype/device
    #     exps = self._exps.to(device=p.device, dtype=p.dtype)        # [k]
    #     inv_exps = (1.0 / exps).to(dtype=p.dtype)

    #     # power means: (E[p^a])^(1/a), for a in exps
    #     pm = torch.pow(p.unsqueeze(-1), exps)       # [B,L,4,N,k]
    #     pm = pm.mean(dim=-2)                        # mean over N -> [B,L,4,k]
    #     prob_topk_approx = torch.pow(pm, inv_exps)  # -> [B,L,4,k]

    #     # keep the original mean feature (like your code)
    #     mean_feat = prob.mean(dim=-1, keepdim=True) # [B,L,4,1]

    #     stat = torch.cat([prob_topk_approx, mean_feat], dim=-1)     # [B,L,4,k+1]
    #     quality_score = self.reg_conf(stat.reshape(B, L, -1))       # [B,L,1]
    #     return scores + quality_score
...
class DFINETransformer(nn.Module):
    ...
    def __init__(
        ...
        print(f"eval_spatial_size: {eval_spatial_size}")
        self.aux_loss = aux_loss
        self.reg_max = reg_max

        assert query_select_method in ("default", "one2many", "agnostic"), ""
        assert cross_attn_method in ("default", "discrete", "custom"), ""
    ...
    def _generate_anchors(
        ...
        for lvl, (h, w) in enumerate(spatial_shapes):
            ...
            grid_xy = (grid_xy.unsqueeze(0) + 0.5) / torch.tensor([w, h], dtype=dtype).reshape(1, 1, 1, 2) # TODO: fix type error for CV error
        ...
        anchors_clamped = anchors.clip(min=self.eps, max=1 - self.eps)
        anchors = torch.log(anchors_clamped) - torch.log(1 - anchors_clamped)
    ...
    def _get_decoder_input(
        ...
        if self.training or self.eval_spatial_size is None:
            ...
            # Eval/deploy mode: prefer cached anchors/valid_mask (built from eval_spatial_size),
            # but fall back to dynamic generation if the cached shapes don't match the current
            # feature map resolutions (e.g. variable input sizes or FLOPs profiling).
            anchors = self.anchors
            valid_mask = self.valid_mask
            if (
                anchors is None
                or valid_mask is None
                or anchors.shape[1] != memory.shape[1]
                or valid_mask.shape[1] != memory.shape[1]
            ):
                anchors, valid_mask = self._generate_anchors(spatial_shapes, device=memory.device)
        if memory.shape[0] > 1:
    ...
    #     if (not self.training) and getattr(self, "deploy_no_topk", False):
    #         # Deterministic selection: take first topk tokens
    #         topk_memory  = memory[:, :topk, :]
    #         topk_anchors = outputs_anchors_unact[:, :topk, :]
    #         topk_logits  = None
    #         return topk_memory, topk_logits, topk_anchors

    #     # original behavior (training / non-deploy)
    #     if self.query_select_method == "default":
    #         _, topk_ind = torch.topk(outputs_logits.max(-1).values, topk, dim=-1)
    #     elif self.query_select_method == "one2many":
    #         _, topk_ind = torch.topk(outputs_logits.flatten(1), topk, dim=-1)
    ...
    #         dim=1, index=topk_ind.unsqueeze(-1).repeat(1, 1, outputs_anchors_unact.shape[-1])
    #     )
    #     topk_logits = None if not self.training else outputs_logits.gather(
    #         dim=1, index=topk_ind.unsqueeze(-1).repeat(1, 1, outputs_logits.shape[-1])
    #     )
    #     topk_memory = memory.gather(
    #         dim=1, index=topk_ind.unsqueeze(-1).repeat(1, 1, memory.shape[-1])
